...
def normalize(kernel, k):
    """normalizes a kernel, which is assumed to be kxk"""
    smallest = kernel[0][0]
    biggest = kernel[0][0]
    for x in range(k):
        for y in range(k):
...
def detect(file):
    ...
    for theta in map(lambda a: math.radians(a*(180/orientations)),
                     range(orientations)):
    ...
    for y_hat in filter(lambda x: x % subsample == 0, range(k/2, height-k/2)):
        print("computing dominant orientation: ", (y_hat-k/2)/subsample+1,
              "of", (height-k)/subsample)
        ...
        for y in filter(lambda _y: angle_vote[x][_y] != -1, range(height)):
            ...
            for top_y in range(min(y, int(0.75*height))):
                ...
                if angle_vote[x][y] > (math.pi/2):
                    start = (y-top_y) * math.tan((angle_vote[x][y]-math.pi/2)
                                                 - (math.pi)/(orientations*2))
                    stop = (y-top_y) * math.tan((angle_vote[x][y]-math.pi/2)
                                                + (math.pi)/(orientations*2))
                    for vote_x in filter(lambda x: x < width and x > 0,
                                         range(int(start)+x, int(stop)+x)):
                        votes[vote_x][top_y] += 1
                else:
                #acute
                    start = (y-top_y) * math.tan((math.pi/2-angle_vote[x][y])
                                                 - (math.pi)/(orientations*2))
                    stop = (y-top_y) * math.tan((math.pi/2-angle_vote[x][y])
                                                + (math.pi)/(orientations*2))
                    for vote_x in filter(lambda x: x < width and x > 0,
                                         range(int(start)-x, int(stop)+x)):
                        votes[vote_x][top_y] += 1
